...
- **D-12:** ALL hardcoded system library deps across the nFPM YAMLs (`libgpgme11`, `libseccomp2`, `libsystemd0`, `libcap2`, `libglib2.0-0`, `libsubid4`, `libsqlite3-0`) replaced by the build-time detected set. *(Historical inventory of what WAS hardcoded pre-v3.0. NOTE: `libsqlite3-0` was a stale skopeo datum — skopeo v1.22.0 is built with no sqlite BUILDTAG and links no sqlite; the on-host UAT falsified it and the skopeo baseline was corrected to `libgpgme11t64 libsubid4` in Plan 05. See 19-UAT.md / debug/detector-transitive-closure.md.)*
...
| A2 | `libseccomp2`, `libsystemd0`, `libcap2`, `libsqlite3-0`, `libsubid4` were NOT t64-renamed on 24.04 | Pitfall 1 | If one WAS renamed, the expected detected-vs-hardcoded delta on 24.04 differs; verify on a real 24.04 host in Wave 0. Detection still produces the correct name regardless. *(t64 claim holds; NOTE: skopeo v1.22.0 links no sqlite — `libsqlite3-0` removed from the skopeo baseline as stale in Plan 05, see 19-UAT.md.)* |
...
2. **Exact 24.04 detected-vs-hardcoded delta (the D-14 expected mapping)**
   ...
   - **RESOLVED (Plan 04 Task 1):** `scripts/verify_depends.sh` encodes the t64-adjusted baseline (libgpgme11t64/libglib2.0-0t64 substituted for the pre-v3.0 hardcoded names; libseccomp2/libsystemd0/libcap2/libsubid4 unchanged per A2) and asserts D-14 as **functional equivalence**, not string identity, on a real ubuntu:24.04 host. Any undocumented t64 delta beyond the two known names is a FAIL, so the on-host run both confirms A2 and records the asserted baseline.
   - **CORRECTED (Plan 05, on-host UAT):** the on-host run falsified the skopeo `libsqlite3-0` assumption — skopeo v1.22.0 (built with no sqlite BUILDTAG) links no sqlite at any depth. The skopeo baseline was corrected to `libgpgme11t64 libsubid4` (libsqlite3-0 removed as stale). Separately, the detector was fixed to derive deps from direct DT_NEEDED sonames only (no transitive closure). See 19-UAT.md / debug/detector-transitive-closure.md.
